...
constructor(){
 ...
 //console.log(contacts);
  ...
  render() {
    return (
      <div className="him">
      <input type="button" id="01" value="Add contact" onClick={()=>{ this.setState({save:0})}} />
      {
        (this.state.save==1)?
      <Display contact={this.state.filteredcontact} ondelete={this.onDelete}/>:
      <div>
      <p>Name:</p>
      <input id="02" onChange={this.onNameChange}/>
      <p>phone:</p>
      <input id="03" onChange={this.onPhoneChange}/>
      <p>Name:{`${this.state.name}`}</p>
      <p>Phone:{`${this.state.phone}`}</p> 
      <input id="04" type="submit" value="Save" onClick={this.onSubmitData} />
      </div>
     }
     </div>
    ); 
